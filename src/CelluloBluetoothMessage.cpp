...
/**
 * @file CelluloBluetoothMessage.cpp
 * @brief Source for Bluetooth messages to/from Cellulo robots
 * @author Ayberk Özgür
 */

#include "CelluloBluetoothMessage.h"

const char* CelluloBluetoothMessage::sendMessageTypeStr[] = {
    "P", //(P)ing
...
const int CelluloBluetoothMessage::sendMessageTypeLen[] = {
    2, //Ping
    "D", //Set broadcast perio(d)
    "I", //Enable (i)mage streaming + disable localization or vice-versa
    "T", //Enable (t)imestamping along with pose and disable idling or vice-versa
...
const char* CelluloBluetoothMessage::receiveMessageTypeStr[] = {
    "O", //B(o)ot finished
...
const int CelluloBluetoothMessage::receiveMessageTypeLen[] = {
    2, //B(o)ot finished
    2, //Wake up finished ((H)ello)
    "D", //All keys long touched; shutting (d)own
    2, //Battery lo(w), shutting down
    "B", //(B)attery state changed
    "T", //(T)ouch began
    "L", //(L)ong touch
    "R", //Touch (r)eleased
...
CelluloBluetoothMessage::CelluloBluetoothMessage(){
    sendMessageType = SEND_MESSAGE_TYPE::INVALID_MESSAGE_TYPE;
    receiveMessageType = RECEIVE_MESSAGE_TYPE::INVALID_MESSAGE_TYPE;
    payload.clear();
}

CelluloBluetoothMessage::~CelluloBluetoothMessage(){ }

void CelluloBluetoothMessage::setSendMessageType(SEND_MESSAGE_TYPE type){
    sendMessageType = type;
}

void CelluloBluetoothMessage::clearPayload(){
    payload.clear();
}

QByteArray CelluloBluetoothMessage::getSendMessage(){
    QByteArray message;

    message.append(PACKET_START_CHAR);
    message.append(sendMessageTypeStr[sendMessageType]);
    message.append(payload);

    return message;
}

void CelluloBluetoothMessage::loadNumber(uint32_t num){
    payload.append(QByteArray::fromRawData((char*)&num, 4));
}

void CelluloBluetoothMessage::loadNumber(uint16_t num){
    payload.append(QByteArray::fromRawData((char*)&num, 2));
}

void CelluloBluetoothMessage::loadNumber(uint8_t num){
    payload.append(QByteArray::fromRawData((char*)&num, 1));
}

void CelluloBluetoothMessage::loadNumber(int32_t num){
    payload.append(QByteArray::fromRawData((char*)&num, 4));
}

void CelluloBluetoothMessage::loadNumber(int16_t num){
    payload.append(QByteArray::fromRawData((char*)&num, 2));
}

void CelluloBluetoothMessage::loadNumber(int8_t num){
    payload.append(QByteArray::fromRawData((char*)&num, 1));
}
